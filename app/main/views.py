...
def user(username):
    ...
    pagination = user.posts.order_by(Post.timestamp.desc()).paginate(
        ...
        error_out=False)
    each_page_posts = pagination.items
    return render_template('user.html', user=user, posts=each_page_posts,
...
def post(id):
    ...
    if form.validata_on_submit():
        comment = Comment(body=form.body.data,
                          post=post,
                          author=current_user._get_crrent_object())
...
def moderate():
    ...
    pagination = Comment.query.order_by(Comment.timestamp.desc()).paginate(
        page, per_page=current_app.config['FLASKY_COMMENTS_PER_PAGE_MODERATE'],
        error_out=False)
    comments = pagination.items
